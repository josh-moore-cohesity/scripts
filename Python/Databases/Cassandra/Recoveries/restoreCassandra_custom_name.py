...
source = [s for s in sources if s['protectionSource']['name'].lower() == sourceserver.lower() or s['protectionSource']['customName'].lower() == sourceserver.lower()]
if source is None or len(source) == 0:
    print('Cassandra protection source "%s" not found!' % sourceserver)
    exit(1)
else:
...
results = [o for o in search['cassandraObjects'] if o['sourceInfo']['name'].lower() == sourcename.lower() and o['name'].lower() == objectname.lower()]

if results is None or len(results) == 0:
    print("Keyspace/Table %s/%s not found" % (sourcename, objectname))
    exit(1)
