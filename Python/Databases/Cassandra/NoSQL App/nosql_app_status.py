...
if(clusterlist is not None):
    appsfile = 'nosql-app-status-%s-%s.csv' % (clusterlist.split(".")[0],dateString)
else:
    appsfile = 'nosql-app-status-%s.csv' % dateString
af = codecs.open(appsfile, 'w', 'utf-8')
af.write('Cluster,App Name,Version,Installed,State,Duration,Health Status,Health Detail\n')
appsreport = []
    ...
    if appsmode['marketplaceAppsMode'] == 'kBareMetal':
        ...
        if totalinstances > 0:
            for runningapp in runningapps:
                ...
                appversion = runningapp['devVersion']
                ...
                appsreport.append(str('%s,%s,%s,%s,%s,%s,%s,%s' % (clusterinfo['name'],appname,appversion,installstate,appstate,formatted_uptime,healthstatus,healthDetail)))
                if healthstatus != 'kHealthy':
                    print('%s,%s,%s' % (appname,healthstatus,healthDetail))
        else:
            appname = "NoSQL and Hadoop Service"
