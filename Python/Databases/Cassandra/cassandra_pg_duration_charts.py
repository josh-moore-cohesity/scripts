#!/usr/bin/env python
"""Graph Cassandra Protection Runs"""
...
for clustername in clusternames:
    ...
    fig_divs = []
    sources = api('get', 'protectionSources?environment=kCassandra')
    jobs = api('get', 'protectionJobs?environments=kCassandra&isDeleted=False&isActive=True')
    if len(jobs) == 0 or jobs == None:
        print("No Cassandra Jobs Found on %s" % clustername)
    for job in jobs:
        jobid = job['id']
        jobname = job['name']
        ...
        if len(runstats) > 0:
            ...
            print("No run stats found for PG %s" % jobname)
            continue
    ...
    #Create HTML File    
    header = '%s - Cassandra PG Duration Trends' % clustername.upper()
    html_content = """
    <!DOCTYPE html>
    <html>
    <head>
    ...
    with open("%s-dse-job_duration_trends.html" % clustername, "w", encoding="utf-8") as f:
